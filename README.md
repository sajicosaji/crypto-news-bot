...
```
Claudeのクラウドルーティン（1日4回: 8:00 / 14:00 / 20:00 / 2:00 JST）
        ↓ ワークフローを起動
GitHub Actions の「自走ジョブ」（1回あたり約5.5時間動き続ける）
        ↓ ジョブの中で自分の時計を見て判断
  ・30分ごと      → 速報チェック
  ・8:00 JST      → 日次まとめ（その日まだ投稿していなければ）
  ・月曜 8:00 JST → 週次振り返り（その週まだ投稿していなければ）
```
...
ジョブが重なっても二重投稿は起きません。逆に朝の実行が失敗しても、14時までに復旧すれば
