...
| 種類 | タイミング | 内容 |
|---|---|---|
| 日次まとめ | 毎朝7:00（日本時間） | 前日のニュースと価格・オンチェーンデータを1チャンネル1投稿（ARBは毎日、他はニュースがある日だけ） |
| 速報 | 30分ごと | 重大ニュース・注目ニュース・急な値動きがあるときだけ即時投稿（1記事1投稿・メンションなし） |
...
該当が無ければ何も書きません（無理に理由をこじつけません）。しきい値は
`move_context.coin_specific_pct`（既定3%）で変えられます。

### 日本語要約（Claude Haiku）

英語の記事も**日本語3〜5行に要約**して表示します。記事ページから本文を取得し、
Claude Haiku（`claude-haiku-4-5`）に要約させています。BOTの中でここだけが有料APIです。

費用を抑えるため、次の3点で制限しています。

- **実際に表示する記事だけ**要約する（一覧に載らない記事には課金しない）
- 一度要約したらDBに保存して**使い回す**（同じ記事に二度課金しない）
- 1回の実行あたりの件数に上限（`summary.max_articles_per_run`、既定15件）

想定費用は月200〜500円程度です。`ANTHROPIC_API_KEY` が未設定だったりAPIが落ちていても、
**自動的に本文抜粋の表示に戻るだけ**でBOTは止まりません。

要約を止めたいときは `config.yaml` の `summary.enabled` を `false` にします。

### できないこと（正直な制約）

- 要約の材料は記事ページから取得した本文です。ページを取得できない媒体では
  抜粋のみが材料になるため、要約が短くなることがあります。
- 記事に書かれていないことは書きません（推測や投資判断は出しません）。
...
日本語要約に使うAPIキーも同じ方法で登録します。

```
gh secret set ANTHROPIC_API_KEY --repo sajicosaji/crypto-news-bot
```

キーは https://console.anthropic.com の API Keys から発行します（Claudeの月額プランとは
別枠の、従量課金のAPIクレジットが必要です）。
...
- `digest.skip_when_no_news`: その日のニュースが1件も無い銘柄は、日次まとめ自体を
  投稿しない（既定 true。静かな日の空投稿を減らすため）
- `digest.always_post`: ニュースが無くても必ず毎日投稿する銘柄（既定 `[ARB]`）。
  価格・Robinhood Chain収益・トレジャリー残高だけでも届きます
- `summary.enabled`: 日本語要約のON/OFF（既定 true）
- `summary.max_articles_per_run`: 1回の実行で要約する最大件数＝費用の上限（既定15）
...
- `notable_media_count`: この媒体数以上が同じニュースを報じていたら、重大ワードが
  無くても「注目ニュース」として個別に速報する（既定3）。朝のまとめまで待たずに
  価値のある話題を届けるための設定です。通知が多いと感じたら数を増やしてください。
...
`mention` で速報のメンションを設定します。**既定はメンションなし（空文字）**です。
全員に通知が飛ぶと迷惑になるためで、速報自体は今までどおり投稿されます。

通知が欲しくなったら `@here`（オンラインの人全員）/ `@everyone`（全員）/
`<@&ロールID>`（特定のロールだけ）に変更できます。自分だけ受け取りたい場合は
専用ロールを作ってそのIDを指定するのが確実です。
